...
class SaveBrowser():
    ...
    def list_command(self):
        ...
        data = tts.describe_files_by_type(self.filesystem,
                                          self.save_type.get())
    ...
    def file_list_has_changed(self, now):
        ...
        if self.save.isInstalled:
            ...
            self.status_label.config(text='Some cache files missing '
                                          '- check details on list page.')
...
class TTS_GUI:
    ...
    def populate_list_frame(self, frame):
        ...
        self.list_sb.bind("<<SelectionChange>>",
                          self.update_list_frame_details)
    ...
    def importPak(self):
        ...
        if rc:
            ...
            messagebox.showwarning("TTS Manager", "Pak import failed.")
    ...
    def populate_export_frame(self, frame):
        ...
        self.export_sb.bind("<<SelectionChange>>",
                            self.update_export_frame_details)
        targetFrame = ttk.Frame(frame)
        targetFrame.pack(expand=Tk.Y, fill=Tk.BOTH)
        self.downloadMissingFiles = Tk.BooleanVar()
        self.downloadMissingFiles.set(False)
        self.downloadMissingFilesCB = ttk.Checkbutton(
            targetFrame,
        ...
        ttk.Button(targetFrame,
                   text="Browse",
                   command=self.pickExportTarget).pack(side=Tk.LEFT)
    ...
    def populate_import_frame(self, frame):
        ...
        importEntryButton = ttk.Button(
            importEntryFrame,
            text="Browse",
            command=self.pickImportTarget)
        importEntryButton.pack(side=Tk.LEFT)
    ...
    def download(self):
        if not self.download_sb.save:
            tts.logger().warn("Internal error: no save when downloading")
    ...
    def download_all(self):
        ...
        for ident in self.download_sb.file_store.values():
            ...
            if not successful:
                break
        if successful:
            messagebox.showinfo("TTS Manager",
    ...
    def __init__(self, root):
        ...
        ttk.Button(pref_frame,
                   text="Preferences",
                   command=self.showPreferences).pack()
